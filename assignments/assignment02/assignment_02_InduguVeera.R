...
## Use `setwd()` if needed
setwd("C:/Users/raghu/Downloads/Veera DS/DSC520/Veera_R_Projects/")
> getwd()
[1] "C:/Users/raghu/Downloads/Veera DS/DSC520/Veera_R_Projects/"
> dir()
[1] "Data"                   "Images"                 "R"                      "Veera_R_Projects.Rproj"

## Load the file `data/tidynomicon/person.csv` to `person_df1` using `read.csv`
## Examine the structure of `person_df1` using `str()`
theUrl <- "data/tidynomicon/person.csv"
person_df1 <- read.csv(file=theUrl,header=TRUE,sep=",")
str(person_df1)
...
person_df2 <- read.csv(file=theUrl,header=TRUE,sep=",",stringsAsFactors=FALSE)
str(person_df2)

## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
scores_df <- read.csv(file="data/scores.csv",header=TRUE,sep=",",stringsAsFactors=FALSE)
summary(scores_df)
